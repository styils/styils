...
describe('StyleSheet', () => {
  ...
  it('should set the data-styils attribute to the key option', () => {
    const key = 'some-key'
    const sheet = new StyleSheet({ ...defaultOptions, key })
    sheet.insert(rule)
    expect(document.documentElement).toMatchSnapshot()
    expect(document.querySelector('[data-styils]')?.getAttribute('data-styils')).toBe(key)
    sheet.flush()
  })
  ...
  it('should set the nonce option as an attribute to style elements', () => {
    ...
    sheet.insert(rule)
    expect(sheet.tags[0]).toBe(document.querySelector('[data-styils]'))
    expect(sheet.tags).toHaveLength(1)
